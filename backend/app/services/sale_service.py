...
from .seller_split import compute_shares
...
async def create_sale(
    ...
    *,
    ...
    The automatic per-seller split is computed from the FIRST item's book
    ``observaciones`` (lowest item order) and stored on the sale. Raises
    ``OversellError`` if any item's stock cannot cover the requested quantity,
    or ``BookUnavailableError`` for a missing/inactive book.
    """
    ...
    first_book_observaciones: str | None = None
    total = Decimal("0.00")
    for index, item in enumerate(items):
        ...
        if index == 0:
            first_book_observaciones = book.observaciones
    ...
    sale.juli_share, sale.cande_share = compute_shares(first_book_observaciones)
    ...
    await log_audit(
        ...
        changes={
            ...
            "juli_share": str(sale.juli_share),
            "cande_share": str(sale.cande_share),
            "items": len(items),
        },
    )
    return sale